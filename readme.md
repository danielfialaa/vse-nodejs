...
Při předávání funkce, jako parametru jiné funkci (auth v new LocalStrategy()) si musíme dát pozor, abychom vynechali klasické závorky (auth()). Pokud bychom v parametrech uvedli funkci společně se závorkami, funkci bychom nepředávali dané funkci jako parametr, tedy že by ji mohla zavolat, ale rovnou by se při inicializaci zavolala.

Dále budeme potřebovat funkci, která nám serializuje uživatele, abychom ho mohli uložit do naší session. K tomu využijeme funkci dostupnou v knihovně Passport a to konkértně passport.serializeUser(). Do parametru si vložíme arrow funkci o dvou parametrech – user a done. To samé, akorát s **id** místo **user**, uděláme pro passport.deserializeUser().
...
function initPassport(passport, getUserByName){
   ...
   . . .
```
Pokud se nyní podíváme do konzole serveru, uvidíme chybu **SyntaxError: Unexpected identifier** v naší podmínce, kde ověřujeme heslo. Problém zde vyvolává **await**, který zde máme kvůli asynchroní funkci z bcryptu. Před výpisem možných parametrů u funkce **auth** tedy připíšeme **async**, čímž definujeme, že se jedná o asynchroní funkci a budeme tak moci použít **await**. Celou tuto podmínku také opět vložíme do try-catch bloku, respektive do **try** a v části **catch** přidáme další volání funkce **done**, kde předáme chybovou hlášku.
...
Když máme nakonfigurovaný Passport, potřebujeme ještě serveru říct, jak jej má použít. Začneme tím, že si na serveru naimportujeme zbylé dvě knihovny, které jsme instalovali (express-session a express-flash). Také musíme nadefinovat, že je má server použít, pomocí funkce app.use().
